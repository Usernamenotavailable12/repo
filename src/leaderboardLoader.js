...
class FullLeaderboardTable {
  ...
  min-height: 200px;
    ...
    this.leaderboardTable = this.parrentElement.querySelector(
      ".leaderboard-table-main"
    );
    this.leaderboardButtonsBox = this.parrentElement.querySelector(
      ".leaderboard-buttons-box"
    );
    this.leaderboardHeader = this.parrentElement.querySelector(
      ".leaderboard-header"
    );
    this.leaderboardBody =
      this.parrentElement.querySelector(".leaderboard-body");
...
C255,161.018,253.42,157.202,250.606,154.389z"/>
  ...
  destroy() {
    ...
    this.parrentElement.removeChild(
      this.parrentElement.querySelector(".leaderboard-table-main")
    );
  ...
  async getAllData() {
    try {
      ...
      for (const leaderboardInfo of this.leaderboardsInfo) {
        const lbPromise = this.getTournamentData(
          leaderboardInfo.id,
          leaderboardInfo.count,
          DateNow
        );
      ...
      this.makeElementDraggable(
        document.getElementsByClassName("leaderboard-buttons-box")[0],
        {
          speedMultiplier: 1.0,
          enableTouch: true,
          dragCursor: "grabbing",
      ...
      if (!hasVerticalScroll(ele)) {
        Array.from(
          document.getElementsByClassName("leaderboard-header-scroll-button")
        ).forEach((btn) => {
  ...
  scrollButtonsRight() {
    ...
    const value = getComputedStyle(this.leaderboardButtonsBox).getPropertyValue(
      "--gap-between-buttons"
    );
    ...
    this.smoothScrollTo(
      this.leaderboardButtonsBox,
      currentScrollLeft + buttonWidth
    );
  }
  ...
  scrollButtonsLeft() {
    ...
    this.smoothScrollTo(
      this.leaderboardButtonsBox,
      Math.max(0, currentScrollLeft - buttonWidth)
    );
  }
  ...
  async getTournamentData(leaderboardId, actionsCount, DateNow) {
    try {
      ...
      if (!tournamentNode) {
        console.warn(
          "Tournament data is missing for leaderboardId:",
          leaderboardId
        );
        return null;
      }
  ...
  async getUserData(leaderboardId, userId, DateNow) {
    try {
      ...
      if (!userNode) {
        console.warn(
          "User data is missing for userId:",
          userId,
          "in leaderboard:",
          leaderboardId
        );
  ...
  async createWebSocket() {
    const socket = new WebSocket(
      "wss://www.ambassadoribet.com/_internal/ws/default/default/"
    );
  ...
  async sendSubscriptions() {
    ...
    subscriptions.forEach((sub) => {
      this.leaderboardTableWebSocket.send(
        JSON.stringify({
          id: sub.tournamentId,
          type: "start",
          payload: {
  ...
  async handleSubscriptionData(data) {
    const userTournament =
      data.payload?.data?.userTournamentUpdatedEx?.userTournament;
    if (userTournament) {
  ...
  updateTableData(userTournament, leaderboardId) {
    const shouldUpdateTable =
      userTournament.points >= this.getLowestPoints(leaderboardId);
  ...
  updateRow(rowElement, userItem, currentUserId) {
    const nicknameElement = rowElement.querySelector(
      ".leaderboard-row-nickname"
    );
    const pointsElement = rowElement.querySelector(".leaderboard-row-points");
    const positionElement = rowElement.querySelector(
    ...
    if (nicknameElement && pointsElement && positionElement) {
      ...
      rowElement.classList.toggle(
        "champion",
        userItem.user.userId === currentUserId
      );
    }
  }
  ...
  updateTableBody(userTournament, leaderboardId) {
    ...
    const tableBody = document.getElementById(
      `leaderboard-table-row-body-${leaderboardId}`
    );
    if (!tableBody) return;
    ...
    const oldPoints =
      userIndex !== -1
        ? leaderboardData.usersData[userIndex].node.points
        : null;

    if (userIndex !== -1) {
      leaderboardData.usersData[userIndex].node.points = userTournament.points;
    } else if (
      leaderboardData.usersData.length < maxCount ||
      userTournament.points >
        (leaderboardData.usersData[leaderboardData.usersData.length - 1]?.node
          .points || 0)
    ) {
    ...
    leaderboardData.usersData.forEach(
      (item, idx) => (item.node.position = idx + 1)
    );
  ...
  handlePointsUpdate(tableBody, usersData, userId, oldPoints, newPoints) {
    ...
    if (pointsElement) {
      const startValue = parseFloat(
        this.getPointsText(oldPoints).replace(/[^0-9.-]+/g, "")
      );
      const endValue = parseFloat(
        this.getPointsText(newPoints).replace(/[^0-9.-]+/g, "")
      );

      if (!isNaN(startValue) && !isNaN(endValue)) {
        this.animateCounter(pointsElement, startValue, endValue, 1000);
      } else {
        console.error("Invalid points values:", {
          oldPoints,
          newPoints,
          startValue,
          endValue,
  ...
  handlePositionChanges(tableBody, usersData, positionChanges) {
    ...
    setTimeout(() => {
      usersData.forEach((item, index) => {
        let rowElement = tableBody.querySelector(
          `#leaderboard-table-row-body-${index + 1}`
        );
        if (rowElement) {
  ...
  updateCurrentUserData(userTournament, leaderboardId) {
    const userRow = document.getElementById(
      `leaderboard-table-user-info-${leaderboardId}`
    );
    if (!userRow) return;
    ...
    this.animateCounter(
      pointsElement,
      parseInt(this.getPointsText(pointsElement.textContent)),
      this.getPointsText(userTournament.points)
    );
    this.animateCounter(
      positionElement,
      parseInt(positionElement.textContent),
    ...
    if (leaderboardData && leaderboardData.rewardsData) {
      ...
      if (userTournament.position <= maxRewardPositions) {
        ...
        rewardElement.className = `leaderboard-row-reward ${this.getTablePrizeClassnames(
          rewardAction
        )}`;
  ...
  getLowestPoints(leaderboardId) {
    const lastIndex =
      this.leaderboardsData.get(leaderboardId)?.usersData?.length - 1;
    return (
      this.leaderboardsData.get(leaderboardId).usersData[lastIndex]?.node
        .points || 0
    );
  }
  ...
  async executeGraphQLQuery(query, variables) {
    ...
    while (true) {
      const response = await fetch(
        `https://www.ambassadoribet.com/_internal/gql/?_${Date.now()}`,
        {
          method: "POST",
  ...
  drawTableHeaderButton(leaderboardId, data) {
    ...
    const leaderboardInfo = this.leaderboardsInfo.find(
      (item) => item.id === leaderboardId
    );
    ...
    button.addEventListener("click", () =>
      this.toggleLeaderboardTable(leaderboardId)
    );
  }
  ...
  drawLeaderboardBody(leaderboardInfo, userInfo, leaderboardId) {
    ...
    if (
      this.currentUserInfo.id !== null &&
      this.currentUserInfo.id !== "" &&
      this.currentUserInfo.id !== undefined
    ) {
  ...
  drawLeaderboardData(data, leaderboardId) {
    ...
    for (
      let i = 0;
      i < Math.max(data.rewardsData.length, data.usersData.length);
      i++
    ) {
      const userId = data.usersData[i]?.node.user.userId || "";
      rowHtML += `
          <div class="leaderboard-table-row ${this.isCurrentUser(
            userId,
            leaderboardId
          )}" 
              ...
              <div class="leaderboard-row-points">
                  ${
                    data.usersData[i]?.node.points
                      ? this.getPointsText(data.usersData[i]?.node.points)
                      : "-"
                  }
              </div>
              <div class="leaderboard-row-reward ${this.getTablePrizeClassnames(
                data.rewardsData[i]?.action
              )}">
  ...
  drawLeaderboardUserInfo(data, leaderboardId) {
    let userRowHTML = `
              <div class="user-info-container">
                  <div class="leaderboard-table-user-info ${
                    this.userData.get(leaderboardId)?.isWinner
                      ? "hide-user-info"
                      : ""
                  }" id="leaderboard-table-user-info-${leaderboardId}">
                      <div class="leaderboard-row-position">
                          ${data?.position || "-"}
                      </div>
                      <div class="leaderboard-row-nickname">
                      ...
                      <div class="leaderboard-row-reward ${this.getTablePrizeClassnames(
                        this.leaderboardsData.get(leaderboardId).rewardsData[
                          data?.position - 1
                        ]?.action
                      )}">
                          ${
                            this.getTablePrizeText(
                              this.leaderboardsData.get(leaderboardId)
                                .rewardsData[data?.position - 1]?.action
                            ) || "-"
                          }
                      </div>
                  </div>
  ...
  toggleLeaderboardTable(leaderboardId) {
    ...
    const button = document.getElementById(
      `leaderboard-button-${leaderboardId}`
    );
    ...
    const newScrollLeft =
      button.offsetLeft - (parentRect.width - buttonRect.width) / 2;
  ...
  getTablePrizeClassnames(data) {
    ...
    const classMap = {
      [`${data?.bonus?.reward?.type === this.REWARD_TYPES.FREE_SPINS}`]:
        "table-prize-free-spins",
      [`${data?.bonus?.reward?.type === this.REWARD_TYPES.FIXED}`]:
        "table-prize-fixed",
      [`${Boolean(data?.title)}`]: data?.title
        ? data?.title.trim().replace(/ /g, "_")
        : "",
      [`${data?.box?.type === this.BOX_TYPES.LOOT_BOX}`]:
        "table-prize-loot-box",
      [`${data?.box?.type === this.BOX_TYPES.MYSTERY_BOX}`]:
        "table-prize-mystery-box",
      [`${data?.box?.type === this.BOX_TYPES.WHEEL_OF_FORTUNE}`]:
        "table-prize-wheel-of-fortune",
    };
  ...
  getTablePrizeText(data) {
    ...
    const prizeTextMap = {
      [`freespins-${Boolean(
        ...
        data?.bonus?.reward?.amount +
        ` <img class="free-spins-icon" src="https://images.takeshape.io/5da2b4d5-59f6-412a-82c3-f6a272b532be/dev/bb35ec84-1583-40a4-9c84-2f0c3948e82b/slot-machine_1f3b0.png" loading="lazy">`,
  ...
  setLoading(loading) {
    ...
    if (!loading) {
      ...
      this.parrentElement.getElementsByClassName(
        "mat-loader"
  ...
  updateLeaderboards() {
    ...
    this.leaderboardsData.forEach((data, index) => {
      ...
      if (currentDate >= startEt && currentDate <= endEt) {
        if (!activeFound) {
          document
            .getElementById(`leaderboard-table-${index}`)
            .classList?.add("active");
          const buttonElement = document.getElementById(
            `leaderboard-button-${index}`
          );
    ...
    if (!activeFound && latestEndDate < currentDate) {
      document
        .getElementById(`leaderboard-table-${latestIndex}`)
        .classList.add("active");
      const buttonElement = document.getElementById(
        `leaderboard-button-${latestIndex}`
      );
      buttonElement.classList.add("active");
      activeButton = buttonElement;
    }
